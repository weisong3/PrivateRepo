...
public class RegisterDialogHandler extends DialogHandler {

	private Dialog dialog;
	private Context mContext;
	private RegisterDialogResult caller;
	private int id = 0;

	public RegisterDialogHandler(Looper looper, Context context, RegisterDialogResult caller) {
		super(looper);
	...
	public void setCaller(RegisterDialogResult caller) {
		this.caller = caller;
	}
	...
	public void handleMessage(final Message msg) {
		...
		case DISPLAY_DIALOG:
			...
			final View view = factory.inflate(R.layout.fragment_register, null);
			final EditText usernameEditText = (EditText) view.findViewById(R.id.register_username_edittext);
			final EditText passwordEditText = (EditText) view.findViewById(R.id.register_password_edittext);
			final EditText repasswordEditText = (EditText) view.findViewById(R.id.register_repassword_edittext);
			...
			builder.setPositiveButton(positiveText, new OnClickListener() {
				public void onClick(DialogInterface dialog, int arg1) {
					dialog.dismiss();
					caller.onRegister(id, usernameEditText.getText(), passwordEditText.getText(), repasswordEditText.getText());
				}
			});
			String negativeText = data.getString(DialogFactory.negativeBtn);
			if (null == negativeText) {
				negativeText = getContext().getResources().getString(R.string.cancel);
			}
	...
	public RegisterDialogResult getCallback() {
		return this.caller;
	}
}
